...
const commentDisplays = ref<CommentChara[]>([]);
...
// アバターのスタイルを取得する関数
const getAvatarStyles = (comment: CommentChara, index: number) => {
 return {
  backgroundColor: comment.chara?.color['--lcv-background-color'],
...
watch(
 ...
 (newComments, oldComments) => {
  ...
  newComments.forEach((comment) => {
   if (!oldIds.has(comment.data.id)) addComment(comment);
  });
 },
 { deep: true }
);
...
// コメント管理ロジック
const addComment = (comment: CommentChara) => {
 if (commentCompIds.has(comment.data.id)) return;

 // 古いコメントがあれば削除
 removeComment(comment.data.id);
...
const removeComment = (id: string) => {
 commentDisplays.value = commentDisplays.value.filter((c) => c.data.id !== id);
 commentCompIds.delete(id);

 if (commentTimers.has(id)) {
