import { ref as N, watch as A, onUnmounted as L, defineComponent as T, computed as P, createElementBlock as x, openBlock as b, Fragment as C, createCommentVNode as j, createElementVNode as g, unref as F, normalizeClass as m, toDisplayString as y, renderList as V } from "vue";
...
const D = new class {
  ...
  run(t, e) {
    try {
      ...
      return e.enableCount > 0 && e.enableCount < ((d == null ? void 0 : d.draws) || 0) || (this.gameState = { ...this.gameState, userStats: a, rankings: n }), { placeholders: this.createPlaceholders(a[s], n, s, e), postActions: [], rankingList: n };
    } catch (s) {
  ...
  updateUserStats(t, e, s) {
    ...
    if (d === "point") {
      ...
      const l = { userId: t, name: e, draws: this.gameState.userRecords[t].length, wins: a ? 1 : 0, points: n, totalPoints: this.gameState.userRecords[t].reduce((c, i) => c + i.points, 0), rank: 0, rate: 0 };
  ...
  generateRankings(t, e, s) {
    ...
    const d = this.createRankingEntries(t), o = n === "rate" ? "rate" : "wins", l = d.sort((i, r) => (r[o] ?? 0) - (i[o] ?? 0)).map((i, r) => ({ ...i, rank: r + 1 })), c = l.findIndex((i) => i.userId === s);
    if (c > 0) {
      const [i] = l.splice(c, 1);
      l.unshift(i);
    }
    return l;
  }
  createRankingEntries(t) {
    ...
    return Object.entries(t).map(([s, a]) => {
      const n = e.find((i) => i.userId === s), d = a.draws ?? (n == null ? void 0 : n.draws) ?? 0, o = a.wins ?? (n == null ? void 0 : n.wins) ?? 0, l = a.points ?? (n == null ? void 0 : n.points) ?? 0, c = a.totalPoints ?? (n == null ? void 0 : n.totalPoints) ?? 0;
      return { userId: s, name: a.name || "Unknown", draws: d, wins: o, points: l, totalPoints: c, rank: a.rank ?? (n == null ? void 0 : n.rank) ?? 0, rate: d > 0 ? o / d * 100 : 0 };
    });
  }
  createPlaceholders(t, e, s, a) {
    var n;
    const { rankMode: d } = this.settings;
    if (d === "point") {
    ...
    const o = a.enableCount > 0 && a.enableCount < t.draws, l = this.calculateRank(t, e, s), c = t.draws > 0 ? ((t.wins || 0) / t.draws * 100).toFixed(1) : "0.0";
    return { announceMessage: o ? `${t.name}さんは上限を超えているから、参考記録だよ。` : `${t.name}さんの順位は、${l}位だよ。`, winsCount: String(t.wins ?? 0), winsRank: l, winsRate: c, totalDraws: String(t.draws ?? 0), totalPoints: String(t.totalPoints ?? 0) };
  }
  calculateRank(t, e, s) {
    if (this.settings.rankMode === "point") {
      const n = t.points ?? 0, d = e.filter((o) => (o.points ?? 0) > n).length;
      return String(d + 1);
    }
...
const H = new class {
  ...
  run(t, e) {
    const { mode: s } = e, { isParty: a, enableCount: n } = this.settings, d = this.executeGame(s), o = this.LogRank.run(t, { enableCount: n, isWin: !1, getPoint: d.points });
    return { postActions: a ? d.postArray : [], placeholders: { ...o.placeholders, message: this.createResultMessage(t, d.points), points: String(d.points) }, rankingList: o.rankingList };
  }
  executeGame(t) {
    const e = q[t];
  ...
  const e = t, s = function(l, c) {
    ...
    const l = e.userRanking[0] || null, c = [...e.userRanking].sort((i, r) => r.points - i.points);
    return { showResult: !!l, result: { score: (l == null ? void 0 : l.points) || 0, name: (l == null ? void 0 : l.name) || "プレイヤー" }, rankPlayers: c.slice(0, e.settings.rankingLimit), totalCount: e.userRanking.length, totalPoint: e.userRanking.reduce((i, r) => i + r.points, 0) };
  }), d = P(() => {
    const l = n.value;
  ...
  return (l, c) => (b(), x(C, null, [F(s).isVisible.value ? (b(), x("div", { key: 0, class: m(["rounded-full mx-auto relative whitespace-nowrap flex flex-col justify-center items-center -mb-8", "bg-gradient-to-br from-blue-50 to-blue-100 shadow-2xl border-4 border-blue-200", "bubble-animation shadow-blue-200/50", a.value.scoreCircle]) }, [g("div", { class: m(["font-bold text-blue-900 font-rocknroll", a.value.text.large]) }, "スコア", 2), g("div", { class: m(["font-bold font-mochiy bg-gradient-to-b from-amber-300 to-amber-600 bg-clip-text text-transparent", a.value.text.xlarge]) }, y(n.value.result.score), 3), g("div", { class: m(["mt-2 font-bold text-blue-900 font-rocknroll", a.value.text.large]) }, y(n.value.result.name), 3)], 2)) : j("", !0), g("div", { class: m(["bg-amber-50 border-4 border-amber-400 rounded-3xl font-rocknroll", a.value.container.padding]) }, [g("div", { class: m(["text-center mb-2 text-amber-800 font-bold drop-shadow-lg", a.value.text.large]) }, " 🍉スイカランキング🍉 ", 2), g("div", K, [g("ul", Q, [(b(!0), x(C, null, V(n.value.rankPlayers, (i, r) => (b(), x("li", { key: r, class: m(o(r)) }, [r < 3 ? (b(), x("img", { key: 0, src: `img/image_${r + 1}.png`, alt: `${r + 1}位`, class: m(["inline-block rounded-full border-2 border-white shadow-md", a.value.rankingImage, a.value.container.imageMargin]) }, null, 10, Y)) : (b(), x("span", { key: 1, class: m(["inline-block font-bold text-center rounded-lg bg-gray-200 text-gray-700", a.value.rankingNumber, a.value.container.imageMargin]) }, y(r + 1), 3)), g("span", Z, [g("span", { class: m(["font-bold mr-2", a.value.text.medium]) }, y(i.points), 3), g("span", { class: m(["text-gray-600", a.value.text.medium]) }, "(" + y(i.name) + ")", 3)])], 2))), 128))])]), g("div", { class: m(["p-0 m-0 flex justify-center items-center w-full text-gray-600 mt-2", a.value.text.small]) }, " 平均: " + y(d.value) + "pt (" + y(n.value.totalCount) + "回 / 総計" + y(n.value.totalPoint) + "pt) ", 3)], 2)], 64));
